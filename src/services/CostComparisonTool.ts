...
import { SupportedChain, SupportedOperation, TransactionResult } from "../types";
...
export class CostComparisonTool {
  ...
  public async run(chains: SupportedChain[], operations: SupportedOperation[]): Promise<void> {
    ...
    if (unhealthyChains.length > 0) {
      console.warn(`⚠️  Warning: Some chains are not responding: ${unhealthyChains.map(hc => hc.chainId).join(', ')}`);
    }

    const healthyClients = clients.filter(({ chainId }) =>
  ...
  private async executeConcurrentOperations(
    ...
    return results
      .filter((result): result is PromiseFulfilledResult<TransactionResult> =>
