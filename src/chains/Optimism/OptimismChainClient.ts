...
export class OptimismChainClient extends AbstractChainClient {
  ...
  private async getOpUsdPrice(): Promise<BigNumber> {
    const avaxUSDPrice = (await this.coinGeckoApiService.getOpPriceInUsd())[
      'optimism'
      ].usd;
  ...
  private async createViemTransactionResult(
    ...
    const l2FeeWei = receipt.gasUsed * receipt.effectiveGasPrice;
    //@ts-ignore
    const l1FeeWei = receipt.l1Fee;

    const totalCostWei = l2FeeWei + l1FeeWei;
    const totalCostEth = formatUnits(BigInt(totalCostWei), 18);
    const usdCost = BigNumber(totalCostEth)
