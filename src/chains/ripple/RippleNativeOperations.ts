...
export class RippleNativeOperations implements IRippleNativeOperations {
  ...
  constructor(
    ...
    this.rippleWalletCredentials =
      this.configService.getRippleWalletCredentials();
  ...
  isHealthy(): Promise<boolean> {
    return new Promise(async (resolve) => {
      try {
        ...
        const _response = await this.rippleClient.request({ command: 'ping' });
        await this.rippleClient.disconnect();
        resolve(true);
      } catch (error) {
        console.error('RippleNativeOperations health check failed:', error);
        try {
          await this.rippleClient.disconnect();
        } catch (_) {}
        resolve(false);
  ...
  private async enableRippling() {
    ...
    const txResponse = await this.rippleClient.submitAndWait(issuerSettings, {
      wallet: account,
    });
    await this.rippleClient.disconnect();
    return txResponse;
  ...
  async mintNativeNFT(): Promise<TxResponse<SubmittableTransaction>> {
    ...
    const nftMintResult = await this.rippleClient.submitAndWait(
      signedNftMint.tx_blob
    );
    await this.rippleClient.disconnect();
  ...
  async createNFTCreateOffer(): Promise<TxResponse<SubmittableTransaction>> {
    ...
    const nftOfferResult = await this.rippleClient.submitAndWait(
      signedNftOffer.tx_blob
    );
    await this.rippleClient.disconnect();
    return nftOfferResult;
  }

  async transferNativeNFT(
    offerId: string
    ...
    const acceptOfferResult = await this.rippleClient.submitAndWait(
      signedAcceptOffer.tx_blob
    );
    await this.rippleClient.disconnect();
  ...
  async hcsSubmitMessage(): Promise<TxResponse<SubmittableTransaction>> {
    ...
    const memoResult = await this.rippleClient.submitAndWait(
      signedMemo.tx_blob
    );
    await this.rippleClient.disconnect();
    return memoResult;
